...
export const FILE_CONTENTS: Record<string, string> = {
  about:
    "About Deshan Chandima — Software Developer. Backend focus: RESTful APIs, MERN/TypeScript, JWT. Exploring AI/ML. Based in Bhubaneswar, India. Type 'cd about' to open the full section.",
  projects:
...
export function formatLsLong(entries: readonly string[]): string {
  ...
  return entries
    .map((name) => `drwxr-xr-x  2 deshan deshan 4096 ${mon} ${day} ${time} ${name}`)
    .join("\n");
}
...
export function getDfOutput(human: boolean): string {
  if (human) {
    return [
      ...
      "/dev/skills      10G  6.5G  3.0G  68% /home/deshan/skills",
    ].join("\n");
  }
  return [
    ...
    "/dev/skills     10485760   6815744   3145728  68% /home/deshan/skills",
  ].join("\n");
}
...
export function getPsOutput(aux: boolean): string {
  if (aux) {
    return [
      ...
      "deshan         1  0.0  0.1  16823  9921 ?        Ss   00:00   0:01 /sbin/init",
      "deshan       420  0.3  2.1 998244 84992 ?        Sl   00:00   0:42 next-server",
      "deshan       777  0.1  1.0 512000 40000 ?        S    00:00   0:07 portfolio-terminal",
      "deshan      1337  0.0  0.5 128000 20000 pts/0    R+   14:23   0:00 ps aux",
    ].join("\n");
  }
  return [
    "    PID TTY          TIME CMD",
...
export function getEnvOutput(user: string): string {
  return [
    ...
    "PORTFOLIO=Deshan Chandima — Software Developer",
  ].join("\n");
}
...
export function getLsbRelease(): string {
  return [
    ...
    "Description:    Deshan Chandima Portfolio Terminal",
    "Release:        2026.05",
...
export function getGitStatus(sub: string): string {
  if (sub === "log") {
    return [
      ...
      "Author: Deshan Chandima <deshan.c163@gmail.com>",
